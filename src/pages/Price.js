            ...
            <div className="bg-white lg:px-32 px-5 py-16">
                ...
                <div className="grid lg:grid-cols-3 gap-12 lg:gap-8">
                    {pricingPlans.map((plan) => (
                        <div key={plan.title}
                             className="relative border border-slate-200 shadow-lg p-8 bg-white rounded-2xl flex flex-col">
                            <h3 className="text-lg font-semibold leading-5">{plan.title}</h3>
                            {plan.mostPopular &&
                                <p className="absolute -top-3 bg-primary text-secondary px-3 py-0.5 text-sm font-semibold tracking-wide rounded-full shadow-md">پرفروش‌ترین</p>}
